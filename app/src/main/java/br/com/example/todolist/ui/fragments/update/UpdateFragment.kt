package br.com.example.todolist.ui.fragments.update
...
class UpdateFragment : Fragment() {
    ...
    private fun updateItem() {
        ...
        if (inputCheck(title, description, date, hour)) {
            ...
            Toast.makeText(
                requireContext(),
                getString(R.string.update_toast),
                Toast.LENGTH_SHORT
            )
                .show()
            findNavController().navigate(R.id.action_updateFragment_to_listFragment)
            ...
            Toast.makeText(
                requireContext(),
                getString(R.string.txt_toast_inputcheckerror),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
    ...
    private fun deleteUser() {
        ...
        alertDialog.setNeutralButton(getString(R.string.list_dialog_cancel)) { _, _ -> }

        alertDialog.setNegativeButton(getString(R.string.list_dialog_no)) { _, _ -> }

        alertDialog.setPositiveButton(getString(R.string.list_dialog_yes)) { _, _ ->
            mTaskViewModel.deleteTask(args.currentTask)
            Toast.makeText(
                requireContext(),
                getString(R.string.update_toast_removed),
                Toast.LENGTH_SHORT
            )
                .show()
            findNavController().navigate(R.id.action_updateFragment_to_listFragment)
